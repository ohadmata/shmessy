...
def test_large_file_infer_should_be_less_than_2000_ms(create_large_file):
    df = pd.read_csv(create_large_file)
    result = Shmessy().infer_schema(df)
    assert result.infer_duration_ms < 2000
...
def test_duration_for_sample_file_should_be_less_than_2000_ms(file_path,files_folder):
